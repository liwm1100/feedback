    ...
    <div class="contentText">
            ...
            <p >
                ...
                <select id="companys" name="companys">
                    ...
                    <c:forEach items="${companyList}" var="item">
                        <option value="${item.code},${item.name}" <c:if test="${store.companyIn == item.code}">selected="selected"</c:if>>${item.code}-${item.name}</option>
                    </c:forEach>
                </select>
            </p>
            <p><p><p>
        ...
        <table id="tbl" class="tablelest" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td><input type="text" id="barNos" name="barNos" value="${store.barNo}"></td>
                <td>
                    <select id="goodsInfos" name="goodsInfos" onchange="changeGoods(this)">
                        <option value="">--请选择--</option>
                        <c:forEach items="${goodsList}" var="item">
                            <option value="${item.goodsNo},${item.goodsName}"<c:if test="${store.goodsNo==item.goodsNo}">selected="selected"</c:if>>${item.goodsNo}-${item.goodsName}</option>
                        </c:forEach>
                    </select>
                </td>
                ...
                <td><input type="text" name="nums" id="nums"  value="${store.num}"></td>
